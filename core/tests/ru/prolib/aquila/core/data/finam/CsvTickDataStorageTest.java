...
public class CsvTickDataStorageTest {
	...
	@Test
	public void testGetTicks_IfNoData() throws Exception {
		descr = new SecurityDescriptor("SBER", "EQBR", ISO4217.RUR, SecurityType.STK);
		ticks = storage.getTicks(descr, new DateTime(1998, 1, 1, 0, 0, 0, 0));
		assertFalse(ticks.next());
	}
	
	@Test
	public void testGetTicks_IfTimeBeforeData() throws Exception {
		List<Tick> actual = new Vector<Tick>(),
				expected = getExpectedAll();
		
		ticks = storage.getTicks(descr, new DateTime( 538, 1, 1, 0, 0, 0, 0));
		while ( ticks.next() ) {
	...
	@Test
	public void testGetTicks_IfTimeAfterData() throws Exception {
		ticks = storage.getTicks(descr, new DateTime(2100, 1, 1, 0, 0, 0, 0));
		assertFalse(ticks.next());
	}
	
	@Test
	public void testGetTicks_TimeAfterEndOfTradingSession_HasNoMoreData()
			throws Exception
	{
		ticks = storage.getTicks(descr, new DateTime(2014, 12, 4, 23, 55, 0, 0));
		assertFalse(ticks.next());
	}
	
	@Test
	public void testGetTicks_TimeAfterEndOfTradingSession_HasMoreData()
			throws Exception
	{
		List<Tick> actual = new Vector<Tick>(),
		...
		ticks = storage.getTicks(descr, new DateTime(2014, 10, 14, 23, 55, 0, 0));
		while ( ticks.next() ) {
			actual.add(ticks.item());
		}
		assertEquals(expected, actual);
	...
	@Test
	public void testGetTicks_ThroughWeekend()
			throws Exception
	{
		List<Tick> actual = new Vector<Tick>(),
				expected = getExpectedAll();
