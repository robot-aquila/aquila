...
public interface Terminal extends Starter, OrderProcessor, Scheduler {
	...
	/**
	 * Добавить обозревателя терминала.
	 * <p>
	 * Этот метод подписывает указанный экземпляр наблюдателя на уведомления
	 * терминала, распространяемые в синхронном режиме. Каждый обозреватель
	 * может быть подписан только единственный раз. Повторные подписки не
	 * приведут к дублирующим уведомлениям.
	 * <p>
	...
	/**
	 * Отписать обозревателя терминала.
	 * <p>
	 * Этот метод отписывает указанный экземпляр наблюдателя от уведомлений
	 * терминала, распространяемых в синхронном режиме. Не подписанные
	 * обозреватели игнорируются.
	 * <p>
	 * @param observer обозреватель
	 */
