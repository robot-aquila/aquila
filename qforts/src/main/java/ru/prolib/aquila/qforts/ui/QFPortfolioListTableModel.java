...
public class QFPortfolioListTableModel extends PortfolioListTableModel {
	...
	protected Object getColumnValue(Portfolio p, int columnID) {
		switch ( columnID ) {
		case CID_VMARGIN:
			return p.getMoney(QFPortfolioField.QF_VAR_MARGIN);
		case CID_VMARGIN_INTER:
			return p.getMoney(QFPortfolioField.QF_VAR_MARGIN_INTER);
		case CID_VMARGIN_CLOSE:
			return p.getMoney(QFPortfolioField.QF_VAR_MARGIN_CLOSE);
		default:
