...
1) Регистрацию клиринга делать через контроллер клиринга - глобальный сервис,
доступ через сервис-локатор.

2) Контроллер клиринга группирует процедуры по времени. При создании нового
стека, регистрирует себя в планировщике. При отработке, контроллер
последовательно вызывает процедуры этого стека, удаляет стек, затем выполняет
post-clearing. По ходу нужен аргумент, так как время на момент отработки может
быть другое. Еще как вариант, чтобы не дублировать функционал планировщика,
декорировать процедуры и использовать аналог CountDownLatch для определения
момента запуска post-clearing процедуры.

3) Для хранения доп атрибутов по позе, использовать позу как ключ.

4) Заявки OrderProcessor трекает сделки. При исполнении (тут подумать) передает
хендлеру инструмента для изменения позы. Следовательно, реестр хендлеров таки
нужен.

5) Параметры позы нужно пересчитывать регулярно. Делать это нужно там, где
